...
const ActiveTab = ({ children }) => <li className="is-active">{children}</li>;
...
module.exports = ({ name, activeTab, handleTabClick }) => [
  <NavItem key="tabs">
    <div className="tabs">
      <ul>
        {tabs.map(tab => {
          const tabLink = <TabLink value={tab} handleClick={handleTabClick} />;
  ...
  <NavItem key="icon">
    <span className="tag is-medium is-info">{name}</span>
  </NavItem>
];
